...
- **Language/Version**: JavaScript (ES Modules in `src/`, transpiled by Babel,
  bundled to CommonJS in `dist/`). Targets modern Node.js (Node 22.17.0+,
...
```text
.
├── dist/                         # Rollup build output (gitignored; generated)
...
├── rollup.config.js              # Bundle src/*.js → dist/ as CJS w/ shebang
...
- You MUST run `yarn build` whenever you change source files that are
  bundled into a CLI entry — it verifies the bundle still produces valid
  CommonJS output. Do NOT commit the regenerated `dist/` artifacts:
  `dist/` is gitignored and every CI workflow rebuilds it before running
  (`yarn install` + `yarn build`), so committed bundles would only go
  stale and cause merge conflicts.
...
```text
CLI entries (src/github-{poll,stats,publish}.js, dist/*)
        ↓
...
All Markdown files MUST comply with the rules defined in
[.markdownlint.json](./.markdownlint.json). Run `yarn lint` to check
for violations; `yarn lint --fix` auto-fixes most of them.

### Other

- **Bundled CLI artifacts**: The contents of [dist/](./dist/) are generated by
  Rollup. Do NOT hand-edit them, and do NOT commit them — `dist/` is
  gitignored because every CI workflow (and the poll-events bot) rebuilds
  it before running. The hashed sibling files (`fs-utils-*.js`,
  `prepare-stats-*.js`) are Rollup chunk artifacts; removing or renaming
  them by hand will break the local binaries — re-run `yarn build` to
