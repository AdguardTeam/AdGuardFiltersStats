...
```text
.
├── bin/                          # Rollup build output (gitignored; generated)
...
- You MUST run `yarn build` whenever you change source files that are
  ...
  `bin/` is gitignored and every CI workflow rebuilds it before running
...
- **Bundled CLI artifacts**: The contents of [bin/](./bin/) are generated by
  Rollup. Do NOT hand-edit them, and do NOT commit them — `bin/` is
